"""!шмальнуть и !топ косячков — в личке и во флуде, зеркало бара.

Каждый вызов — случайные 1–10. Как только сумма чата добивает 50 — косяк
скурен: счётчики в ноль и час тишины, а вызов отвечает «Готовим косячок».
Все 50 в одно лицо — соло-бан 2 часа. Топ — общий по всем чатам.
"""
import html
import random
...
from database.shmal_dao import ShmalDAO
...
EMPTY_TEXT = "Готовим косячок"
SOLO_TEXT = "всё скурил весь косяк, следующая порция без тебя"
...
@shmal_router.message(Exact("!шмальнуть"))
async def shmal_cmd(message: Message):
    now = msk_now()
    uid = message.from_user.id
    name = _name(message.from_user)
    async with async_session_maker() as session:
        dao = ShmalDAO(session)
    ...
    left = EMPTY_AT - total
    await message.reply(
        f"🌿 {name} шмальнул {personal} "
        f"{plural(personal, 'раз', 'раза', 'раз')} 💨\n"
        f"До косячка: {left} {plural(left, 'затяжка', 'затяжки', 'затяжек')}",
        parse_mode="HTML",
    )


@shmal_router.message(Exact("!топ косячков"))
async def shmaltop_cmd(message: Message):
    async with async_session_maker() as session:
        top = await ShmalDAO(session).top()
    if not top:
        await message.reply("Косяк цел и нетронут. Начни с !шмальнуть 🌿")
        return
    lines = ["🌿 <b>Топ косячков</b>", DIVIDER]
    for i, row in enumerate(top, 1):
        name = html.escape(row.display or "без имени")
        lines.append(
            f"{i}. {name} — {row.total} "
            f"{plural(row.total, 'раз', 'раза', 'раз')} 💨")
